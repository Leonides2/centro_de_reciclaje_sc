...
class _UsersPageState extends State<UsersPage> {
  List<User> users = [
    ...
    User(id: 2, name1: "María", lastName1: "Rodríguez", email: "maria@example.com"),
  ];
...
void _addUser(String name,String lastName1, String lastName2, String email) {
  setState(() {
    users.add(User(id: users.length + 1, name1: name,  lastName1: lastName1, lastName2: lastName2, email: email));
  });
}
            ...
            return Card(
              ...
              child: ListTile(
               ...
               leading: CircleAvatar(backgroundColor: Colors.blue, ),
                title: Text(users[index].name1, style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  onPressed: () async {
    final updatedUser = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => UserFormPage(user: users[index], isEditing: true), // 👈 Ahora entra en modo edición
      ),
    );
...
IconButton(
  ...
  onPressed: () {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => UserFormPage(user: users[index]), // 👈 Abre detalles en modo solo lectura
      ),
    );
  },
),
